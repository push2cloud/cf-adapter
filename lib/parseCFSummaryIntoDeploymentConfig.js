...
module.exports = (summaryJSON) => {
  ...
  _.forEach(summaryJSON.services, (service) => {
    var s = {
      name: service.name,
      guid: service.guid,
      type: service.service_plan ? service.service_plan.service.label : undefined,
      plan: service.service_plan ? service.service_plan.name : undefined
    };
    if (service.last_operation) {
      s.lastOperation = {
        type: service.last_operation.type,
        state: service.last_operation.state
      };
    }
    actual.services.push(s);
  });
